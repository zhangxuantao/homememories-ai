...
**P1 — 完善用户体验**
- [x] 设置页"一键处理"按钮 (扫描 → Embedding → 人脸检测 → 聚类 → 事件生成自动串联)
- [x] 视频关键帧提取与详情页视频播放 (支持 mp4/mov/avi/mkv 等格式)
- [x] 时间线事件默认展开、缩略图独立点击、返回路径修正
- [x] 局域网手机访问 (相对路径 API + Vite `--host` 暴露)
...
- [ ] 首页"最近添加"使用 `ORDER BY date_added DESC` 真实数据 (当前用随机照片凑数)
