...
**P0 — 必备基础体验**
- [ ] 手机上传 (底部"+"按钮 → 批量选照片/视频 → 进度条 → 自动触发扫描+AI处理)
- [ ] 扫码访问 (设置页生成局域网二维码，手机一扫即开)
- [ ] 批量操作 (长按进入选择模式，批量加入相册/下载/删除)
- [ ] 重复照片支持删除操作 (当前仅展示对比，无操作入口)

**P1 — 核心体验闭环**
- [ ] 收藏功能 (Lightbox 加收藏按钮，首页加收藏卡片流)
- [ ] 手动相册 (创建/命名/换封面/添加照片，与 AI 事件互补)
- [ ] 分享/导出 (局域网临时分享链接、选中多张导出 zip、拼图)
- [ ] 外部导入 (Google Takeout zip 导入、iCloud 导出目录一键迁移)
- [ ] 系统信息展示 GPU 状态 (CUDA 可用性、显存使用、模型加载状态)

**P2 — 情感与留存**
- [ ] 那年今日 PWA 通知 (每日浏览器推送，提醒回忆)
- [ ] 自动精选集 (基于质量分数 + 人脸 + 事件代表性的月选)
